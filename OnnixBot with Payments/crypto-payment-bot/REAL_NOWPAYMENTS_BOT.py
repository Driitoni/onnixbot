#!/usr/bin/env python3
"""
ENHANCED POCKET OPTION BOT WITH NOWPAYMENTS INTEGRATION
...
if not NOWPAYMENTS_API_KEY or not NOWPAYMENTS_IPN_SECRET:
    print("⚠️  NOWPayments credentials not found. Bot will run in demo mode.")
    DEMO_MODE = True
else:
    DEMO_MODE = False
...
class NOWPaymentsAPI:
    ...
    def get_supported_currencies(self):
        ...
        try:
            response = requests.get(f"{self.base_url}/currencies", headers=self.headers, timeout=30)
            data = response.json()
    ...
    def get_estimate(self, amount, from_currency='usd', to_currency='btc'):
        ...
        try:
            ...
            if data.get('success'):
                return {
                    'estimated_amount': data['result']['estimated_amount'],
                    'source_amount': data['result']['source_amount']
                }
            return None
            
        except Exception as e:
            logger.error(f"Estimate fetch error: {e}")
            return None
    
    def create_payment(self, amount_usd, pay_currency, user_id, description="Bot Premium Subscription"):
        """Create a NOWPayments payment"""
        try:
            # For demo mode, generate mock payment
            if DEMO_MODE:
                transaction_id = f"demo_{user_id}_{int(time.time())}"
                payment_address = f"1DemoAddress{pay_currency.upper()}{user_id}Demo"
                ...
                return {
                    ...
                    'pay_amount': amount_usd * self.get_demo_rate(pay_currency),
                    'pay_currency': pay_currency.upper(),
                    'order_id': f"premium_{plan_id}_{user_id}",
                    'qr_code_url': qr_code_url,
                    'demo_mode': True
                }
            
            # Get estimated amount in chosen currency
            estimate = self.get_estimate(amount_usd, 'usd', pay_currency)
            if not estimate:
                logger.error("Failed to get estimate")
                return {"error": "Failed to get crypto estimate"}
            ...
            payload = {
                ...
                'pay_amount': estimate['estimated_amount'],
                'order_id': f"premium_{int(time.time())}_{user_id}",
            ...
            if data.get('success'):
                ...
                return {
                    ...
                    'pay_amount': float(result['pay_amount']),
                ...
                logger.error(f"NOWPayments API error: {data}")
                return {"error": data.get('error', 'Unknown error')}
                
        except Exception as e:
            logger.error(f"NOWPayments payment creation error: {e}")
            return {"error": str(e)}
    ...
    def get_payment_status(self, payment_id):
        ...
        try:
            ...
            if data.get('success'):
                return {
                    'status': data['result']['order_status'],
                    'actual_amount': data['result'].get('actually_received_at_fiat'),
                    'confirmations': data['result'].get('actually_received_at_blockchain')
                }
            return None
    ...
    def get_demo_rate(self, currency):
        """Get demo exchange rates for testing"""
        rates = {
            'btc': 0.000025,    # ~$30,000 BTC
            'eth': 0.0005,      # ~$2,000 ETH
            'usdt': 1.0,        # USDT = USD
            'ltc': 0.4,         # ~$75 LTC
            'bch': 0.05         # ~$600 BCH
        }
        return rates.get(currency.lower(), 0.000025)
...
class CryptoPaymentBot:
    def __init__(self, token: str):
        ...
        # Supported cryptocurrencies (NOWPayments supports 300+)
        self.crypto_currencies = ['BTC', 'ETH', 'USDT', 'LTC', 'BCH', 'SOL', 'ADA', 'MATIC', 'SHIB']
        ...
        if not DEMO_MODE:
            self.nowpayments_api = NOWPaymentsAPI(NOWPAYMENTS_API_KEY, NOWPAYMENTS_IPN_SECRET)
        else:
            self.nowpayments_api = None
    ...
    def create_payment_request(self, telegram_id: int, plan_id: str, crypto_currency: str) -> dict:
        ...
        try:
            ...
            payment_result = self.nowpayments_api.create_payment(
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (telegram_id, plan_id, payment_result['pay_amount'], 
                  payment_result['pay_currency'], payment_result['payment_id'], 
                  payment_result['pay_address'], "pending"))
            self.db.commit()
            
            return {
                "transaction_id": payment_result['payment_id'],
                "amount_crypto": payment_result['pay_amount'],
                "crypto_currency": payment_result['pay_currency'],
                "payment_address": payment_result['pay_address'],
                "qr_code": payment_result['qr_code_url'],
...
"""
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "select_pair":
            ...
            transaction_id = query.data.replace("simulate_payment_", "")
            await self.simulate_payment(query, context, transaction_id)
...
"""
    ...
    async def show_crypto_selection(self, query, context, plan_id):
        ...
        for crypto in self.crypto_currencies:
            # Calculate approximate crypto amount (demo rates)
            if crypto == 'BTC':
                ...
                crypto_amount = plan['price_usd'] * 0.02
            elif crypto == 'ADA':
                crypto_amount = plan['price_usd'] * 1.5
            elif crypto == 'MATIC':
                crypto_amount = plan['price_usd'] * 2.0
            elif crypto == 'SHIB':
                crypto_amount = plan['price_usd'] * 500000
            else:
                crypto_amount = plan['price_usd'] * 0.000025
            
            keyboard.append([
                InlineKeyboardButton(
                    f"{crypto} (≈{crypto_amount:.6f})" if crypto_amount >= 1 else f"{crypto} (≈{crypto_amount:.2e})",
    ...
    async def create_payment(self, query, context, plan_id: str, crypto_currency: str):
        ...
        if payment_data.get('demo_mode'):
            text += f"4. For demo mode: click the button below to simulate payment"
...
"""
    ...
    async def run(self):
        ...
        # Start polling
        logger.info("🚀 Starting NOWPayments Crypto Bot...")
        if DEMO_MODE:
            logger.warning("⚠️  Running in DEMO MODE - No real payments will be processed")
        else:
            logger.info("✅ Live mode with NOWPayments integration")
